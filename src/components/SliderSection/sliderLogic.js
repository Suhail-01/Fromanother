...
import { vertexShader, fragmentShader } from "../WebGLScene/shaders";
...
export async function initSlider() {
  ...
  const uniforms = {
    ...
    uCrossfadeWidth: {
      value: rippleConfig.crossfadeWidth,
    },

    uMobile: {
      value: window.innerWidth <= 1000 ? 1.0 : 0.0,
    },
  };
  ...
  function handleResize() {
    ...
    uniforms.uMobile.value =
      window.innerWidth <= 1000 ? 1.0 : 0.0;

    rippleConfig.endValue =
      getMaxCornerDist() + rippleConfig.waveWidth;

    rippleConfig.duration =
      window.innerWidth <= 1000 ? 1.5 : 3.0;
  }
  ...
  function transition() {
    ...
    exitTimeline.then(() => {
      ...
      const nextSlide =
        buildSlideContent(slides[nextIndex]);
